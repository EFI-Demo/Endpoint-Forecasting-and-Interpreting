...
def read_graphs_in_networkx(infile, labels=True, num_graphs_to_read=10000000):
    data = open(infile, "r", encoding="utf-8").read().splitlines()
    index = 0
    graphs = []
    graphs_name = []
    while index < len(data):
        ...
        if index < len(data) and len(item) > 0 and item[0] == '#' and len(graphs) < num_graphs_to_read:
            graphs_name.append(item.split('\\')[-1])
            g = nx.Graph()  # 无向图
            ...
            for i in range(num_nodes):
                ...
                index = index + 1
                if labels:
                    g.add_node(i, c=0, candidate=[], classification=node_label)
            ...
            for j in range(num_edges):
                ...
                if labels:
                    g.add_edge(start_node, end_node, label=edge_label)
                else:
...
def read_graphs_in_networkx2(infile, labels=True, num_graphs_to_read=10000000):
    data = open(infile, "r", encoding="utf-8").read().splitlines()
    index = 0
    graphs = []
    node_label_dict = {}
